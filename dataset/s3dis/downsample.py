...
def random_sample(
    coords: np.ndarray, colors: np.ndarray, semantic_labels: np.ndarray, instance_labels: np.ndarray, ratio: float
):
...
def get_parser():
    ...
    parser.add_argument("--data-dir", type=str, default="./preprocess", help="directory save processed data")
    parser.add_argument("--ratio", type=float, default=0.25, help="random downsample ratio")
    parser.add_argument(
        "--voxel-size", type=float, default=None, help="voxelization size (priority is higher than voxel-size)"
    )
...
if __name__ == "__main__":
    ...
    for data_file in glob.glob(osp.join(data_dir, "*.pth")):
        ...
        if voxelize_flag:
            ...
            coords, colors, semantic_labels, instance_labels = random_sample(
                coords, colors, semantic_labels, instance_labels, args.ratio
            )
