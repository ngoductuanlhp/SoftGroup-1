...
from .blocks import MLP, PositionalEmbedding, ResidualBlock, UBlock
...
def non_maximum_cluster(
    box_conf, coords, pt_offsets, pt_offsets_vertices, batch_offsets, radius=6**2, mean_active=300, iou_thresh=0.3
):
    # box_conf: N
    ...
    for i in range(len(proposals_sort_indices)):
        ...
        proposals_idx_final.append(
            torch.stack(
                [
                    torch.ones_like(proposals_indices[proposals_sort_indices[i]]) * cluster_id,
                    proposals_indices[proposals_sort_indices[i]],
                ],
                -1,
            )
        )
...
def non_maximum_cluster2(
    ...
    # breakpoint()
    return (
        proposals_idx_final.cpu().int(),
        proposals_offset_final.cpu().int(),
        proposals_box_final,
        proposals_conf_final,
    )
...
def superpoint_align(spp, proposals_pred):
    ...
    sum_spp_inst = torch_scatter.scatter(
        proposals_pred, spp_ids.expand(n_inst, n_points), dim=-1, reduce="sum"
    )  # n_inst, n_spp
