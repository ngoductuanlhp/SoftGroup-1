...
class Criterion(nn.Module):
    ...
    def cal_point_wise_loss(
        ...
        if total_pos_inds == 0:
            ...
            offset_loss = F.l1_loss(pt_offsets[pos_inds], pt_offset_labels[pos_inds], reduction="sum") / total_pos_inds
            ...
            iou_gt, giou = giou_aabb(
                pt_offsets_vertices[pos_inds], pt_offset_vertices_labels[pos_inds], coords_float[pos_inds]
            )
    ...
    def single_layer_loss(
        ...
        for b in range(batch_size):
            ...
            # breakpoint()
            loss_dict["focal_loss"] = loss_dict["dice_loss"] + compute_sigmoid_focal_loss(
                mask_logit_pred, inst_label, num_gt_batch
            )
            ...
            # print('pt_offsets_vertices', pt_offsets_vertices.shape)
            loss_dict["box_loss"] = (
                loss_dict["box_loss"] + F.l1_loss(box_pred, box_label, reduction="sum") / num_gt_batch
            )
    ...
    def forward(self, batch_inputs, model_outputs, epoch=0):
        ...
        gt_dict, aux_gt_dict = self.matcher.forward_dup(
            ...
            instance_labels_,
            batch_offsets_,
            instance_label_shift=self.label_shift,
            dup_gt=4,
        )
        ...
        for l in range(n_layers - 1):
            interm_loss_dict = self.single_layer_loss(
                mask_logits_layers[l],
                cls_logits_layers[l],
                conf_logits_layers[l],
                box_preds_layers[l],
                row_indices,
                cls_labels,
                inst_labels,
        ...
        """ Auxilary loss """
        for l in range(n_layers - 1):
            aux_interm_loss_dict = self.single_layer_loss(
                mask_logits_layers[l],
                cls_logits_layers[l],
                conf_logits_layers[l],
                box_preds_layers[l],
