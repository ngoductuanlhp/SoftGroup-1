...
@torch.no_grad()
def cal_geodesic_single(gpu_index, pre_enc_inds, locs_float_, batch_offset_, max_step=32, neighbor=32, radius=0.1, n_queries=128):
...
@torch.no_grad()
def cal_geodesic_vectorize(gpu_index, pre_enc_inds, locs_float_, batch_offsets_, proposals_batch_idxs_, batch_size, max_step=128, neighbor=64, radius=0.05, n_queries=128):
    ...
    for b in range(batch_size):
        ...
        inst_inds = torch.nonzero(proposals_batch_idxs_==b).view(-1)

        if end -start == 0 or len(inst_inds) == 0:
            continue
